...
public class Algorithm {
	...
	public static void main(String[] args) {
		...
		FitnessLandscape land1 = new FitnessLandscape(10, 10);
	...
	// Fields
	public int LOG_LEVEL; // for dev purposes
	...
	public int populationSize; // number of individuals in a generation/population
	public int genomeLength; // number of genes in an individual
	public int generations; // number of generations
	public int currGeneration; // the current generation
	public int learningTrials; // learning occurs in a lifetime, i.e. in between two consecutive generations
	...
	public ArrayList<String> condition; // set of controls and hyperparameters, condition[0] specifies evolutionary
										// search
	public int[][] initialGenotypes;
	public int[][] genotypes; // genotype[i] is an individual, genotype[i][i] is a gene
	...
	public int numUnchangedPhenotype; // phenotype = individual
	public int numChangedPhenotype;

	public int fitCalledCount;
	public int randomWalkCount; // exploration
	public int hillClimbCount; // exploitation
	public int steepestHillClimbCount;
	public int hillClimbSuccessCount;
	public int steepestHillClimbSuccessCount;
	...
	// Constructor
	public Algorithm(int populationSize, int genomeLength, FitnessLandscape landscape, int generations,
			Double crossoverProb, Double mutateProb, int learningTrials, ArrayList<String> condition,
			Boolean calculateDiversity, Double overlapTolerance, int LOG_LEVEL) {
	...
	public Double[][] fitnessStats() {
		...
		int[] bestIndividual = this.genotypes[arrMgr.array1dMaxIndex(this.fitness)];
	...
	// stepRandomWalk()
	public int[] stepRandomWalk(int[] phenotype, int stepSize) {
		...
		while (indexList.size() < stepSize) {
			...
			int geneIndxToFlip = randomizer.nextInt(this.genomeLength);
	...
	// stepHillClimb()
	public int[] stepHillClimb(int[] integers, int stepSize, int attempts) {
		...
		double bestFitness = this.landscape.fitness(integers);
		int[] bestPhenotype = this.arrMgr.copyArray1d(integers);

		for (int a = 0; a < attempts; a++) {

			// construct new phenotype, that will climb up the gradient
			int[] newPhenotype = this.arrMgr.copyArray1d(integers);
			...
			double newFitness = this.landscape.fitness(newPhenotype);
